...
static std::string getHash(std::string Filename) {
  ...
  Hash.update(initData);
  Hash.update(fileBuf.get()->getBuffer());
  auto result = Hash.final();
  return convertToHex(result);
}

void DependencyCollector::addToGitBomFile(std::string Filename) {
  // GITBOM TODO: Keep BomFileName as a member of DependencyCollector ?
  // We should not be updating files here, but instead be able to use the
  // Dependencies list for computing the .gitbom file later in CodeGen.
  // This is only a temporary hack -- to use <source filename>.gitbom as
  // a way to pass the list of dependencies.
  // std::string BomFileName = Dependencies.front() + ".gitbom";
  // size_t len = BomFileName.length();
  // BomFileName.erase(len-9,2);
  SmallString<128> BomFilename = StringRef(Dependencies.front());
  llvm::sys::path::replace_extension(BomFilename, ".gitbom");
  std::error_code EC;
  // Note that this file is opened in Append Mode.
  // This file gets renamed to its git-hash in the end (AsmPrinter).
  llvm::raw_fd_ostream OS(BomFilename, EC, llvm::sys::fs::OF_Append);
  if (EC) {
    llvm::errs() << "\n Unable to open " << BomFilename << " for writing";
    return;
  }
  // GITBOM TODO: Should we also create a .gitbom file storing the names
  // of files besides the hash (for human readability) ?
  auto result_llvm_hash = getHash(Filename);
  if (!result_llvm_hash.empty())
    OS << result_llvm_hash << "\n";
  else
    llvm::errs() << "Unable to getHash for " << Filename;
}
