...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // GITBOM TODO:
  // Should we implicitly pass -MD when using -frecord-gitbom ?
...
#if 0
    // GITBOM TODO: Should the .gitbom file be named after the -o <filename>
    // if -o is passed in the command line ?
    // DependencyCollector doesn't know this name, so we are working around
    // by using <source filename>.gitbom. This is useful if we want to
    // create a <file>.o.gitbom with the file name besides the hash.
    if (Arg *OutputOpt = Args.getLastArg(options::OPT_o)) {
      SmallString<128> OutputFilename(OutputOpt->getValue());
      llvm::sys::path::replace_extension(OutputFilename, "gitbom");
      CmdArgs.push_back(Args.MakeArgString(OutputFilename));
    } else
#endif
    CmdArgs.push_back(
        Args.MakeArgString(Twine(getBaseInputStem(Args, Inputs)) + ".gitbom"));
  }
