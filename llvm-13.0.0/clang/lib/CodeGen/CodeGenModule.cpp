...
// GITBOM TODO:
// Should this be moved to common utils ?
...
// GITBOM TODO:
// 1. Instead of reading dependencies from a temporary .gitbom file, we should
//    make the dependencies list accessible here.
// 2. The hashes of the dependencies should be sorted and stored in a file and
//    the hash of this file will be used as its name. This file (with the name
//    as its hash should be stored in a special dir (.objects/<hash> ?).
void CodeGenModule::EmitGitBomMetadata() {
  llvm::NamedMDNode *GitBomMetadata =
      TheModule.getOrInsertNamedMetadata(".bom");
  std::string GitBomFile = getCodeGenOpts().RecordGitBom;
  StringRef sha_result;
  llvm::SHA1 Hash;

  // Get hash of <file>.o.gitbom
  llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> fileBuf =
      llvm::MemoryBuffer::getFile(GitBomFile, /*IsText=*/true);
  if (fileBuf) {
    StringRef initData =
        "blob " + std::to_string(fileBuf.get()->getBufferSize()) + '\0';
    Hash.update(initData);
    Hash.update(fileBuf.get()->getBuffer());
    sha_result = Hash.final();
  } else
    sha_result = StringRef();

  llvm::LLVMContext &Ctx = TheModule.getContext();
  llvm::Metadata *GitBomNode[] = {
      llvm::MDString::get(Ctx, convertToHex(sha_result))};
  GitBomMetadata->addOperand(llvm::MDNode::get(Ctx, GitBomNode));
  std::error_code EC =
      llvm::sys::fs::rename(GitBomFile, convertToHex(sha_result));
  if (EC)
    llvm::errs() << "\n cannot rename file " << GitBomFile << " to "
                 << sha_result;
}
